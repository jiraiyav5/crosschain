...
const contract = '0x56a81ab5587203d75e41679895b7c4e274ae48ae'
...
function writeLogs(filename, rows) {
  ...
  for (const row of rows) {
   ...
   // res += row.transactionIndex + ','
    ...
    if(row.token.toUpperCase() == '0xf6ec87dfe1ed3a7256cc0c38e3c8139103e9af3b'.toUpperCase())
      {
        res += 'GENE' + ','
        gene_total += (row.receipts/1000000000000000000)
      }
      else if ( row.token.toUpperCase() == '0xdac17f958d2ee523a2206206994597c13d831ec7'.toUpperCase())
        {
          res += 'USDT' +','
          usdt_total += (row.receipts/1000000)
        }
        else if ( row.token.toUpperCase() == '0xb1dF7CE84253ffcd01D92fA6662e761f86b61982'.toUpperCase())
        {
          res += 'DNA' +','
          dna_total += (row.receipts/10000)
        }
      else
        res += row.token + ','
...
async function main() {
  ...
  let last_block_height = rows.length > 0 ? rows[rows.length - 1][0] : 12433130
